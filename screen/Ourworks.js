  ...
  return (
    <Box mb={"4rem"}>
      <Heading m={{base: "4rem 0", md: "4rem 3rem", lg: "4rem 9rem"}} textAlign={{base: "center", md: "start"}} whiteSpace={"nowrap"}>Our Previous Works</Heading>
        ...
        {services.map(item => <Box key={item.id} width={{base: 280, md: 200, lg: 200, xl: 350}}>
            <Link href={`/media/${item.id}`} ><Image src={item.src} alt={item.alt} width={{base:500, md: 400}} height={{base: 320, md: 300, lg: 400}}  borderRadius={"0.9rem"}/></Link>
